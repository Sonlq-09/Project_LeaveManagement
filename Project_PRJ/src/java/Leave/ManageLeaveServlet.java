...
@WebServlet("/manage-leave")
public class ManageLeaveServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (request.getSession().getAttribute("user_id") == null || !"manager".equalsIgnoreCase((String) request.getSession().getAttribute("role"))) {
        ...
        if (leave != null && "Pending".equals(leave.getStatus()) && user != null && "employee".equalsIgnoreCase(user.getRole())) {
            if ("approve".equals(action)) {
                ...
                leave.setRejectReason(rejectReason != null ? rejectReason : "Request rejected by Manager");
            }
